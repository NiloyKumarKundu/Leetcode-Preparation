...
int smallestCommonElement(vector<vector<int>> arr, int m, int n) {
    rep(j, 0, n) {
		int minCommon = arr[0][j];
		int i = 1;
		for (; i < m; i++) {
            if (binarySearch(arr[i], minCommon)) {
				continue;
			} else {
				break;
			}
...
void solve() {
	...
	cout << smallestCommonElement(arr, m, n) << endl;
}
